...
**卡片视觉重做，引流从水印改为末尾单张。** 第一版是深色底 + 每张卡左上「TLDR 科技日报」、左下域名。这在小红书上会被判定成营销号硬导流。改成米白极简、正文卡不带任何品牌标识，引流集中到最后一张 outro 卡（一句话 + 网址 + 「完全免费」）。配图版式也从「全图铺底压暗叠字」改成「上图下文」，图不再需要压暗。每篇的正文卡从 3 张压到 2 张，单张文字量翻倍。

**新增 `scripts/xhs/image_finder.py`。** 主项目 `image_extractor.py` 用的简单 UA 会被 NYT、Bloomberg 直接 403。补一套完整浏览器请求头重抓 og:image，只对选中的候选发请求。没有动主项目的 extractor——那会影响网站本身，超出这次范围。
